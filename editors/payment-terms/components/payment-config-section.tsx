import { useState, useCallback } from "react";
import {
  Settings,
  DollarSign,
  Milestone,
  Clock,
  Pencil,
  Check,
  X,
} from "lucide-react";
import { TextInput, Select } from "@powerhousedao/document-engineering";
import { toast } from "@powerhousedao/design-system/connect";
import type {
  PaymentTermsState,
  PaymentCurrency,
  PaymentModel,
} from "../../../document-models/payment-terms/gen/types.js";
import type { PaymentTermsAction } from "../../../document-models/payment-terms/gen/actions.js";
import { type actions as paymentTermsActions } from "../../../document-models/payment-terms/index.js";

interface PaymentConfigSectionProps {
  state: PaymentTermsState;
  dispatch: (action: PaymentTermsAction) => void;
  actions: typeof paymentTermsActions;
}

const CURRENCY_OPTIONS = [
  { value: "USD", label: "USD - US Dollar" },
  { value: "EUR", label: "EUR - Euro" },
  { value: "GBP", label: "GBP - British Pound" },
];

const PAYMENT_MODEL_OPTIONS = [
  { value: "MILESTONE", label: "Milestone-Based" },
  { value: "TIME_AND_MATERIALS", label: "Time & Materials" },
];

export function PaymentConfigSection({
  state,
  dispatch,
  actions,
}: PaymentConfigSectionProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [formData, setFormData] = useState({
    currency: state.currency || "USD",
    paymentModel: state.paymentModel || "MILESTONE",
    totalAmount: state.totalAmount?.value?.toString() || "",
  });

  const handleSave = useCallback(() => {
    dispatch(
      actions.setBasicTerms({
        proposer: state.proposer,
        payer: state.payer,
        currency: formData.currency,
        paymentModel: formData.paymentModel,
        totalAmount: formData.totalAmount
          ? {
              value: parseFloat(formData.totalAmount),
              unit: formData.currency,
            }
          : undefined,
      }),
    );

    toast("Payment configuration updated", { type: "success" });
    setIsEditing(false);
  }, [formData, state, dispatch, actions]);

  const handleCancel = useCallback(() => {
    setFormData({
      currency: state.currency || "USD",
      paymentModel: state.paymentModel || "MILESTONE",
      totalAmount: state.totalAmount?.value?.toString() || "",
    });
    setIsEditing(false);
  }, [state]);

  const formatAmount = (value: number | undefined, currency: string) => {
    if (!value) return null;
    return new Intl.NumberFormat("en-US", {
      style: "currency",
      currency: currency,
      minimumFractionDigits: 0,
      maximumFractionDigits: 2,
    }).format(value);
  };

  const PaymentModelIcon =
    state.paymentModel === "MILESTONE" ? Milestone : Clock;

  return (
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden transition-shadow hover:shadow-md">
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 bg-stone-50">
        <div className="flex items-center gap-3">
          <div className="flex items-center justify-center w-9 h-9 rounded-lg bg-white text-slate-500">
            <Settings size={18} />
          </div>
          <div>
            <h2 className="text-[15px] font-semibold text-slate-800">
              Payment Configuration
            </h2>
            <p className="text-[13px] text-slate-400 mt-0.5">
              Currency, model, and total amount
            </p>
          </div>
        </div>
        {!isEditing ? (
          <button
            type="button"
            className="inline-flex items-center gap-2 px-3 py-1.5 text-[13px] font-medium text-slate-500 bg-transparent border-none rounded-md cursor-pointer transition-colors hover:bg-stone-100 hover:text-slate-800"
            onClick={() => setIsEditing(true)}
          >
            <Pencil size={14} />
            Edit
          </button>
        ) : (
          <div className="flex gap-2">
            <button
              type="button"
              className="inline-flex items-center gap-2 px-3 py-1.5 text-[13px] font-medium text-white bg-emerald-600 border-none rounded-md cursor-pointer transition-colors hover:bg-emerald-700"
              onClick={handleSave}
            >
              <Check size={14} />
              Save
            </button>
            <button
              type="button"
              className="inline-flex items-center gap-2 px-3 py-1.5 text-[13px] font-medium text-slate-500 bg-transparent border-none rounded-md cursor-pointer transition-colors hover:bg-stone-100 hover:text-slate-800"
              onClick={handleCancel}
            >
              <X size={14} />
              Cancel
            </button>
          </div>
        )}
      </div>

      <div className="p-5">
        <div className="grid grid-cols-3 gap-4">
          {/* Currency */}
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] font-medium text-slate-500">
              <DollarSign size={14} className="inline align-middle mr-1.5" />
              Currency
            </label>
            {isEditing ? (
              <Select
                value={formData.currency}
                onChange={(value) =>
                  setFormData({
                    ...formData,
                    currency: value as PaymentCurrency,
                  })
                }
                options={CURRENCY_OPTIONS}
              />
            ) : (
              <p className="text-[15px] py-2.5">
                <span className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium rounded-full bg-stone-100 text-slate-500">
                  {state.currency}
                </span>
              </p>
            )}
          </div>

          {/* Payment Model */}
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] font-medium text-slate-500">
              <PaymentModelIcon
                size={14}
                className="inline align-middle mr-1.5"
              />
              Payment Model
            </label>
            {isEditing ? (
              <Select
                value={formData.paymentModel}
                onChange={(value) =>
                  setFormData({
                    ...formData,
                    paymentModel: value as PaymentModel,
                  })
                }
                options={PAYMENT_MODEL_OPTIONS}
              />
            ) : (
              <p className="text-[15px] py-2.5">
                <span
                  className={`inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium rounded-full ${state.paymentModel === "MILESTONE" ? "bg-blue-100 text-blue-600" : "bg-amber-100 text-amber-600"}`}
                >
                  {state.paymentModel === "MILESTONE"
                    ? "Milestone-Based"
                    : "Time & Materials"}
                </span>
              </p>
            )}
          </div>

          {/* Total Amount */}
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] font-medium text-slate-500">
              Total Contract Value
            </label>
            {isEditing ? (
              <TextInput
                type="number"
                value={formData.totalAmount}
                onChange={(e) =>
                  setFormData({ ...formData, totalAmount: e.target.value })
                }
                placeholder="0.00"
                step="0.01"
              />
            ) : (
              <p
                className={`text-lg font-mono font-semibold py-2.5 ${!state.totalAmount ? "text-slate-400 italic text-[15px] font-normal" : "text-slate-800"}`}
              >
                {formatAmount(state.totalAmount?.value, state.currency) ||
                  "Not set"}
              </p>
            )}
          </div>
        </div>
      </div>
    </div>
  );
}
